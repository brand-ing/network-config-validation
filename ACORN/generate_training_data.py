...
def generate_dataset(num_configs, output_csv="config_features.csv"):
    ...
    try:
        ...
        print("Make sure parser.py and feature_extraction.py are in the same directory.")
        
        # Create a basic CSV with just the filenames and labels
        basic_data = [{'filename': f, 'secure': int(security_labels[f])} 
                     for f in security_labels.keys()]
        df = pd.DataFrame(basic_data)
        df.to_csv(output_csv, index=False)
        
        print(f"Generated {num_configs} configurations")
        print(f"Saved basic dataset (without features) to {output_csv}")
        print("You'll need to run feature extraction separately.")
